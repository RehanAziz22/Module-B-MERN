...
function Signup() {
    ...
    let signUp = async () => {
        setLoader(true)
        await signUpUser({ email, password, userName, contact })
            .then((success) => {
                setLoader(false)
    ...
    return (
        <Container sx={{ marginTop: "100px" }}>
            <Box sx={{
                ...
                />
                <Button fullWidth sx={{ backgroundImage: "linear-gradient(to right, #F3429A , #AA44B1);", color: "white" }} onClick={signUp}>{isloading ? <CircularProgress color="inherit" /> : "Signup"}</Button>
